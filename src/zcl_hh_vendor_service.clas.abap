CLASS zcl_hh_vendor_service DEFINITION
  PUBLIC FINAL
  CREATE PRIVATE
  GLOBAL FRIENDS zcl_hh_obj_factory_purch.

  PUBLIC SECTION.
    INTERFACES zif_hh_vendor_service.

    ALIASES get_vendor_rating FOR zif_hh_vendor_service~get_vendor_rating.
...
CLASS zcl_hh_vendor_service IMPLEMENTATION.
  METHOD constructor.
    mo_random_int = cl_abap_random_int=>create( seed = cl_abap_random=>seed( )
                                                min  = 1
                                                max  = 5 ).
  ENDMETHOD.
  ...
  METHOD get_vendor_rating.
    ...
    ELSE.
      ...
      INSERT VALUE sty_vendor_rating( vendor        = iv_vendor
                                      vendor_rating = rv_vendor_rating ) INTO TABLE mt_vendor_rating_buffer.
    ENDIF.
  ENDMETHOD.
ENDCLASS.
